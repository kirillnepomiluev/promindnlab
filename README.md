...
В файле `.env` укажите настройки подключения и другие параметры приложения. Новая переменная `PARTNER_PROGRAM_URL` определяет ссылку на партнёрский сервис. Если она задана, в меню бота появится кнопка **«Партнёрская программа»**, открывающая указанное веб-приложение.
